import { Link } from "react-scroll"
import { useSeasonTheme } from "../hooks/useSeasonTheme";

const AboutSection = function () {

    const theme = useSeasonTheme();

    return (
        <section className="min-h-screen flex flex-col justify-center p-3 md:p-20 items-center">
        {/* <section className="bg-[url('https://images.pexels.com/photos/33109/fall-autumn-red-season.jpg')] min-h-screen flex flex-col justify-center p-20"> */}
            <div className="lg:w-3xl p-3 text-white font-bold">
                {/* <h1 className="text-5xl pb-3">Hi! I'm <span className="text-transparent bg-clip-text font-extrabold bg-gradient-to-r from-[#C8005B] to-red-500">Vincenza</span>,</h1> */}
                <h1 className="text-5xl pb-3">Hi! I'm <span 
                className="text-transparent bg-clip-text font-extrabold bg-gradient-to-r from-[#226500] to-[#7FF942]"
                style={{
                        backgroundImage: `linear-gradient(to right, ${theme.nameColors[0]}, ${theme.nameColors[1]})`
                    }}>
                        Vincenza</span>,</h1>
                <p className="text-justify font-medium">About two years ago I started helping some friends modify some .lua files with some friends. While they found the code challenging, I quickly realized it came naturally to me. After a while I began experimenting with some HTML, CSS and Javascript for fun, and I realised that this could be more than a little hobby. After a period of self-studying I decided to attend a bootcamp to better understand where this path could take me, this is the reason why I chose a Full-Stack course, I wanted to know more about the whole ordeal. I keep on working on projects and expanding my knowledge daily.</p>
            </div>
            <Link to="projects" smooth={true} className="cursor-pointer hover:translate-y-2 transition duration-500 ease-in-out">
                <img src="https://img.icons8.com/?size=100&id=37218&format=png&color=ffffff" alt="arrow-down-icon" width={50} />
            </Link>
        </section>
    )
}

export default AboutSection